 ...
 *
 * @author Paulo Roma, Raphael Novello
 * @since 27/09/2016
...
/**  @type {Matrix4} */
var headMatrix = new Matrix4().setTranslate(0, 7.5, 0);

var legMatrix = new Matrix4().setTranslate(-2, -8, 0);
var legLeftMatrix = new Matrix4().setTranslate(2, -8, 0);
var kneeMatrix = new Matrix4().setTranslate(0, -6, 0);
...
var handMatrixLocal = new Matrix4().setScale(3, 3, 3);
...
function handleKeyPress(event) {
    ...
    switch (ch) {
        ...
        case "l":
            headAngle += 15;
            headMatrix.setTranslate(0, 7.5, 0).rotate(headAngle, 0, 1, 0);
            break;
        case "L":
            headAngle -= 15;
            headMatrix.setTranslate(0, 7.5, 0).rotate(headAngle, 0, 1, 0);
        ...
        case 'k':
            if(kneeAngle <= -120) break;
            kneeAngle -= 10;
            var currentkneeRot = new Matrix4()
                .setTranslate(0, 3.4, 0)
                .rotate(-kneeAngle, 1, 0, 0)
                .translate(0, -3.4, 0);
            kneeMatrix.setTranslate(0, -6, 0).multiply(currentkneeRot);
            break;
        ...
        case 'p':
            ...
            var currentLegRot = new Matrix4()
                ...
                .translate(0, -4.5, 0);
            legMatrix.setTranslate(-2, -8, 0).multiply(currentLegRot);
            legLeftMatrix.setTranslate(2, -8, 0).multiply(currentLegRot);
            break;
...
function renderCube(matrixStack, matrixLocal) {
    ...
    gl.uniform4f(loc, 0.4, .4, .5, .6);
    loc = gl.getUniformLocation(lightingShader, "lightPosition");
    gl.uniform4f(loc, 4.0, 10.0, 5.0, 1.0);
